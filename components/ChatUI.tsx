...
export default function ChatUI() {
  const [messages, setMessages] = useState<Message[]>(() => {
    ...
    try {
      return JSON.parse(localStorage.getItem("sparkle:messages") || "[]")
    } catch {
      return []
    }
  })
  ...
  useEffect(() => {
    localStorage.setItem("sparkle:messages", JSON.stringify(messages))
  }, [messages])
  ...
  async function send(input: string) {
    ...
    // Surface the actual error text so we can debug quickly
    if (!res.ok || !res.body) {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-8rem)]">
      ...
      <div className="flex-1 overflow-y-auto pr-1">
        {messages.length === 0 && (
          <div className="text-sm text-neutral-500 mb-3">
            Say hi to Sparkle ✨ — try a quick prompt below or type your own.
          </div>
        )}
        {messages.map((m, i) => (
          <MessageBubble
      ...
      <ControlsBar systemPrompt={systemPrompt} setSystemPrompt={setSystemPrompt} onSend={send} onUpload={onUpload} />
